...
impl Network{
    pub fn new(layers:Vec<usize>,activation:Activation,learning_rate:f32)->Self{
        ...
        for i in 0..layers.len()-1 {
            ...
            let _= wm.iter_mut().map(|mut m| *m += rng.gen::<f32>()).collect::<Vec<_>>();
            let _=bm.iter_mut().map(|mut m| *m += rng.gen::<f32>()).collect::<Vec<_>>();
            w.push(wm);
            b.push(bm);
        }
        Network{
    ...
    pub fn feed_forward(&mut self, inputs:Array2<f32>)->Array2<f32>{
        ...
        for i in 0..self.layers.len()-1 {
            ...
            let _:Vec<_>=current.iter_mut().map(|mut m| *m = (self.activation.function)(m)).collect();
...
fn main() {
    ...
    let activation= Activation{
        function: |x:&f32|->f32{if *x>0.{return *x}else{0.}},
        derivation: |x:&f32|->f32{if *x>0.{return 1.0}else{0.}}
    };
    let mut NN = Network::new(vec![2,4,4,1],activation,0.01);
    println!("NOW TRAINING:");
    //NN.print();
    let inputs = vec![Array2::from(vec![[0.0],[0.0]]),Array2::from(vec![[1.0],[0.0]]),Array2::from(vec![[0.0],[1.0]]),Array2::from(vec![[1.0],[1.0]])];
    let outputs =  vec![Array2::from(vec![[0.0]]),Array2::from(vec![[0.0]]),Array2::from(vec![[0.0]]),Array2::from(vec![[1.0]])];
    println!("before{:?}",NN.feed_forward(Array2::from(vec![[0.0],[1.0]])));
    NN.train(inputs,outputs,3000);
    println!("after{:?}",NN.feed_forward(Array2::from(vec![[0.0],[1.0]])));
    println!("AFTER TRAINING:");
    //NN.print();
    println!("1 1 {}",NN.feed_forward(Array2::from(vec![[1.0],[1.0]])).into_raw_vec()[0]);
    println!("0 1 {}",NN.feed_forward(Array2::from(vec![[0.0],[1.0]])).into_raw_vec()[0]);
    println!("1 0 {}",NN.feed_forward(Array2::from(vec![[1.0],[0.0]])).into_raw_vec()[0]);
    println!("0 0 {}",NN.feed_forward(Array2::from(vec![[0.0],[0.0]])).into_raw_vec()[0]);
